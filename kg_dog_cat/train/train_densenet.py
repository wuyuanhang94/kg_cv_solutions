...
data_path = os.path.join(os.path.curdir, '../input/train')
full_dataset = MyDataset(data_path=data_path, train=True)
...
validate_loader = torch.utils.data.DataLoader(validate_set, batch_size=8, num_workers=8)
...
if args.resume:
    ...
    checkpoint = torch.load('../checkpoint/dog-cat-densenet.pth')
...
def validate(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('../checkpoint'):
            os.mkdir('../checkpoint')
        torch.save(state, '../checkpoint/dog-cat-densenet.pth')
        best_acc = acc

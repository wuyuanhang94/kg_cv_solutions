...
if args.resume:
    ...
    checkpoint = torch.load('../checkpoint/dog-cat-googlenet.pth')
...
def train(epoch):
    ...
    for batch_idx, (inputs, labels) in enumerate(train_loader):
        ...
        loss = loss_ + 0.3 * loss1 + 0.3 * loss2
        loss.backward()
        optimizer.step()
...
def validate(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/dog-cat-googlenet.pth')
        best_acc = acc

...
data_transforms = {
    "train": A.Compose([
        ...
        A.RandomResizedCrop(height=640, width=704, scale=(0.3, 1), interpolation=cv2.INTER_NEAREST),
        ...
        A.CoarseDropout(max_holes=25, max_height=CFG.img_size[0]//40, max_width=CFG.img_size[1]//40,
                         min_holes=15, fill_value=0.18, mask_fill_value=0, p=0.8),
        ], p=1.0),
...
def prepare_loaders(fold, debug=False):
    ...
    train_loader = DataLoader(train_dataset, batch_size=CFG.train_bs if not debug else 20, 
                              num_workers=16, shuffle=True, pin_memory=False, drop_last=False)
    valid_loader = DataLoader(valid_dataset, batch_size=CFG.valid_bs if not debug else 20, 
                              num_workers=16, shuffle=False, pin_memory=False)
...
for fold in range(0, 1):
    ...
    model = load_model("/home/yiw/gi/best_epoch-00.bin")
    optimizer = optim.Adam(model.parameters(), lr=CFG.lr, weight_decay=CFG.wd)
    scheduler = fetch_scheduler(optimizer)

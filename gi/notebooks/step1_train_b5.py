...
class CFG:
    ...
    exp_name      = 'Baselinev2'
    comment       = 'unet-efficientnet_b5-448x448'
    model_name    = 'Unet'
...
data_transforms = {
    "train": A.Compose([
        ...
        A.RandomResizedCrop(*CFG.img_size, scale=(0.1, 1.0), interpolation=cv2.INTER_NEAREST),
        ...
        A.CoarseDropout(max_holes=28, max_height=CFG.img_size[0]//25, max_width=CFG.img_size[1]//25,
                         min_holes=8, fill_value=1.0, mask_fill_value=0, p=0.8),
        ], p=1.0),
...
for fold in range(1, 2):
    ...
    model     = build_model()
    # model = load_model("/home/yiw/gi/last_epoch-00.bin")
    optimizer = optim.Adam(model.parameters(), lr=CFG.lr, weight_decay=CFG.wd)
    scheduler = fetch_scheduler(optimizer)

...
class CFG:
    ...
    exp_name      = 'Baselinev2'
    comment       = 'unet-efficientnet_b3-640*640'
    model_name    = 'Unet'
...
data_transforms = {
    "train": A.Compose([
        ...
        A.CenterCrop(height=576, width=600, p=1.0),
        A.RandomResizedCrop(height=512, width=576, scale=(0.1, 1), interpolation=cv2.INTER_NEAREST),
        ...
        A.CoarseDropout(max_holes=20, max_height=CFG.img_size[0]//25, max_width=CFG.img_size[1]//25,
                         min_holes=10, fill_value=1.0, mask_fill_value=0, p=0.6),
        ], p=1.0),
    
    "valid": A.Compose([
        A.Resize(*CFG.img_size, interpolation=cv2.INTER_NEAREST),
        A.CenterCrop(height=512, width=576, p=1.0),
        ], p=1.0)
}
...
def criterion(y_pred, y_true):
    # return 0.4*BCELoss(y_pred, y_true) + 0.4*TverskyLoss(y_pred, y_true) + 0.2*DiceLoss(y_pred, y_true)
...
for fold in range(1, 2):
    ...
    model     = build_model()
    # model = load_model("/home/yiw/gi/models/effb1/best_epoch_fold1_224.bin")
    optimizer = optim.Adam(model.parameters(), lr=CFG.lr, weight_decay=CFG.wd)
    scheduler = fetch_scheduler(optimizer)

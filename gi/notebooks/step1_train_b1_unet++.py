...
class CFG:
    ...
    exp_name      = 'Baselinev2'
    comment       = 'unetplusplus-efficientnet_b1-768*768_640*704'
...
data_transforms = {
    "train": A.Compose([
        ...
        A.CenterCrop(height=660, width=714, p=1.0),
        A.RandomResizedCrop(height=640, width=704, scale=(0.08, 1), interpolation=cv2.INTER_NEAREST),
        ...
        A.CoarseDropout(max_holes=30, max_height=CFG.img_size[0]//30, max_width=CFG.img_size[1]//30,
                         min_holes=10, fill_value=0.98, mask_fill_value=0, p=0.5),
        ], p=1.0),

    "valid": A.Compose([
        A.Resize(*CFG.img_size, interpolation=cv2.INTER_NEAREST),
        A.CenterCrop(height=640, width=704, p=1.0),
        ], p=1.0)
}
...
def criterion(y_pred, y_true):
    # return 0.1*BCELoss(y_pred, y_true) + 0.5*JaccardLoss(y_pred, y_true) + 0.4*DiceLoss(y_pred, y_true)
...
for fold in range(3, 4):
    ...
    model     = build_model()
    # model = load_model("/home/yiw/gi/models/effb1/best_epoch-fold4_crop_768_9037.bin")
    optimizer = optim.Adam(model.parameters(), lr=CFG.lr, weight_decay=CFG.wd)
    scheduler = fetch_scheduler(optimizer)

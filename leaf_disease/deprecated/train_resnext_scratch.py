...
net = ResNext101_32x4d(num_classes=5)
net = net.to(device)
print(net)
if device == 'cuda':
...
if args.resume:
    ...
    checkpoint = torch.load(os.path.join(cwd, 'checkpoint/leaf-resnext.pth'))
...
def validate(epoch):
    ...
    if acc > best_acc:
        ...
        torch.save(state, os.path.join(cwd, 'checkpoint/leaf-resnext.pth'))
        best_acc = acc

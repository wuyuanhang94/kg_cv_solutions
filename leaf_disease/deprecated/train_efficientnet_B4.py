...
def validate(val_loader, net, optimizer, criterion, fold):
    ...
    if acc > best_acc:
        save_model(net, fold, 'acc', acc)
    if test_loss < best_loss:
        save_model(net, fold, 'loss', loss=test_loss)
...
def main_loop():
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if os.path.isfile(f'checkpoint/leaf-b4-fold{fold}-loss.pth'):
            best_loss = torch.load(f'checkpoint/leaf-b4-fold{fold}-loss.pth')['loss']

        optimizer = optim.SGD(net.parameters(), lr=cfg['lr'], momentum=0.9, weight_decay=cfg['weight_decay'])

...
def get_train_transforms():
    return Compose([
            ...
            OneOf([
                ...
                Cutout(num_holes=2, max_h_size=int(0.1*cfg['img_size']), max_w_size=int(0.1*cfg['img_size']), p=0.5),
            ], p=0.3),
...
def save_model(net, fold, acc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(cwd, f'checkpoint/leaf-b5-fold{fold}-acc.pth'))
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            net = EfficientNet.from_name(cfg['model_arch'], num_classes=5, image_size=cfg['img_size']).to(device)
            net._fc = nn.Linear(net._fc.in_features, 5)
            net = torch.nn.DataParallel(net, device_ids=[0, 1] if device_name == 'TITAN V' else [0])
            net = net.cuda()            
            
            checkpoint = torch.load(f'checkpoint/leaf-b5-fold{fold}-acc.pth')
            ...
            net = EfficientNet.from_pretrained(cfg['model_arch'], num_classes=5, image_size=cfg['img_size']).to(device)
            net._fc = nn.Linear(net._fc.in_features, 5)
            net = torch.nn.DataParallel(net, device_ids=[0, 1] if device_name == 'TITAN V' else [0])
            net = net.cuda() 

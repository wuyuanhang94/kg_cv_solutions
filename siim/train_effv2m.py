...
label_df = pd.read_csv("/home/yiw/siim/train_df.csv")
...
cfg = {
    'fold_num': 5,
    'seed': 2021,
    'model_arch': 'tf_efficientnetv2_m',
    'img_size': 512,
    'epochs': 12,
    'train_batch_size': 32,
    'val_batch_size': 64,
...
def get_train_transforms():
    return Compose([
            ...
            RandomResizedCrop(cfg['img_size'], cfg['img_size'], scale=(0.6, 1.0)),
...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/siim-study-{model_arch}-fold{fold}-512.pth'))
...
def main_loop(resume):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            checkpoint_path = f'checkpoint/siim-study-{model_arch}-fold{fold}-512.pth'

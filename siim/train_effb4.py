...
label_path = "/home/yiw/siim/train_df.csv"
...
def get_train_transforms():
    return Compose([
            ...
            Cutout(num_holes=15, max_h_size=int(0.08*cfg['img_size']), max_w_size=int(0.08*cfg['img_size']), p=0.5),
...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/siim-study-{model_arch}-pre-fold{fold}-768.pth'))
...
def main_loop(resume):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            checkpoint_path = f'checkpoint/siim-study-{model_arch}-pre-fold{fold}-768.pth'

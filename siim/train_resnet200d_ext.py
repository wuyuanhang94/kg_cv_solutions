...
data_path = os.path.join(project_path, 'tmp')
train_path = os.path.join(data_path, 'train')

external_path = "/home/yiw/siim/external/external.csv"
label_df = pd.read_csv(external_path)
...
def get_train_transforms():
    return Compose([
            ...
            Cutout(num_holes=5, max_h_size=int(0.08*cfg['img_size']), max_w_size=int(0.08*cfg['img_size']), p=0.5),
...
best_auc = 0.
target_cols = label_df.iloc[:, 1:5].columns.tolist()
...
class CustomResNet200D(nn.Module):
    def __init__(self, model_name='resnet200d', pretrained=True):
        super().__init__()
        self.model = timm.create_model(model_name, pretrained=pretrained)
        n_features = self.model.fc.in_features
        self.model.fc = nn.Linear(n_features, 4)
...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/siim-study-{model_arch}-ext-fold{fold}-640.pth'))
...
def main_loop(resume):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        net = CustomResNet200D(pretrained=True)
        ...
        if resume:
            checkpoint_path = f'checkpoint/siim-study-{model_arch}-ext-fold{fold}-768.pth'

...
external_path = "/raid/yiw/siim/external/external.csv"
...
def get_train_transforms():
    return Compose([
            ...
            Cutout(num_holes=10, max_h_size=int(0.1*cfg['img_size']), max_w_size=int(0.1*cfg['img_size']), p=0.4),
...
def prepare_dataloader(train_idx, val_idx):
    ...
    df = pd.concat([train_df, ext_df], ignore_index=True)
    val_df = label_df.loc[val_idx, :].reset_index(drop=True)

    train_dataset = SiimDataset(df=df, transforms=get_train_transforms())
...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/siim-study-{model_arch}-fold{fold}.pth'))
...
def main_loop(resume):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            checkpoint_path = f'checkpoint/siim-study-{model_arch}-fold{fold}.pth'

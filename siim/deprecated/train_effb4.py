...
label_path = "/raid/yiw/siim/train_df.csv"
...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/siim-study-{model_arch}-fold{fold}-768.pth'))
...
def main_loop(resume):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            checkpoint_path = f'checkpoint/siim-study-{model_arch}-fold{fold}-768.pth'

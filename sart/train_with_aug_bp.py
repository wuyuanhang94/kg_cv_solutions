...
if split_mode == "valid20":
    ...
    train_inds = train_meta[train_meta["fold"]!=fold_selected].id
    valid_inds = train_meta[train_meta["fold"]==fold_selected].id
    ...
    df_valid = train_df[train_df.id.isin(valid_inds)]
    DatasetCatalog.register(
        Data_Resister_valid,
        lambda: get_Cell_data_dicts(
            imgdir,
...
cfg.SOLVER.IMS_PER_BATCH = 16 #(2 is per defaults)
...
cfg.TEST.EVAL_PERIOD = len(DatasetCatalog.get(Data_Resister_training)) // cfg.SOLVER.IMS_PER_BATCH  # Once per epoch

...
cfg = {
    ...
    'lr': 6e-5,
    'min_lr': 2e-5,
    'loss_weights': [0.95, 0.05], # dice, bce
    'accum_iter': 1,
    'weight_decay': 1e-6,
    'num_workers': 12,
    'device': 'cuda',
...
def validate(fold, val_loader, net, criterion):
    ...
    with torch.no_grad():
        ...
        if val_dice / (batch_idx + 1) > best_dice:
            ...
            torch.save(state, f'checkpoint/hubmap-{model_name}-{encoder_name}-fold{fold}-dice.pth')
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            model_name = cfg['model_name']
            encoder_name = cfg['encoder_name']
            checkpoint = torch.load(f'checkpoint/hubmap-{model_name}-{encoder_name}-fold{fold}-dice.pth')

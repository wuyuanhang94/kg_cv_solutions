import os
from albumentations.augmentations.transforms import Blur, MedianBlur, RandomGamma
...
cwd = os.path.abspath('.')
data_path = os.path.join(cwd, 'hubmap256')
...
def prepare_dataloader(train_ids, val_ids):
    train_dataset = HuBMAPDataset(train_ids, train=True, tfms=train_transforms if cfg['hard_trms'] else light_transforms)

...
best_dice = .0
mean = np.array([0.63931826, 0.47279168, 0.68424988])
std = np.array([0.15543535, 0.22316029, 0.13584446])
...
train_transforms = A.Compose([
    ...
    A.Cutout(num_holes=10, max_h_size=int(.1*cfg['img_size']), max_w_size=int(.1*cfg['img_size']), p=0.25),
], p=1.0)
...
def prepare_dataloader(train_ids, val_ids):
    ...
    val_dataset = HuBMAPDataset(val_ids, train=False, tfms=A.Resize(256, 256))
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        net = smp.Unet(encoder_name=cfg['encoder_name'], encoder_weights='imagenet', classes=1, activation=None)
        if resume:
            model_name = cfg['model_name']
            encoder_name = cfg['encoder_name']
            img_size = cfg['img_size']

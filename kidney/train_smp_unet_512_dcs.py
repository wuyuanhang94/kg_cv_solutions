...
cfg = {
    ...
    'lr': 5e-4,
    'min_lr': 1e-4,
    'loss_weights': [0.1, 1.9], # dice, bce/focal
    'accum_iter': 1,
    'weight_decay': 1e-6,
    'num_workers': 12,
    'device': 'cuda',
...
def seed_everything(seed):
    ...
    torch.backends.cudnn.benchmark = True # True brings some stochasticity, but 10% speed up

best_dice = .0
mean = np.array([0.63990417, 0.4734721, 0.68480998])
std = np.array([0.16061672, 0.22722983, 0.14034663])
...
class DiceBCEFocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets, smooth=1):
        ...
        return dice_loss, bce_loss, focal_loss
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            net = smp.Unet(encoder_name=cfg['encoder_name'], encoder_weights=None, classes=1, activation=None, aux_params=aux_params)
            net = torch.nn.DataParallel(net, device_ids=cfg['device_num'])
            net = net.cuda()
            ...
            encoder_name = cfg['encoder_name']
            img_size = cfg['img_size']
            checkpoint = torch.load(f'checkpoint/pretrained-{model_name}-{encoder_name}-{img_size}-dice.pth')
            net.load_state_dict(checkpoint['net'])

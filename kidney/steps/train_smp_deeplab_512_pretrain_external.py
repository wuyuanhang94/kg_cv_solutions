...
cwd = os.path.abspath('.')
data_path = os.path.join(cwd, 'external')
images_path = os.path.join(data_path, 'images_1024')
masks_path = os.path.join(data_path, 'masks_1024')
...
cfg = {
    ...
    'lr': 1e-4,
    'min_lr': 5e-6,
    'loss_weights': [2.6, 0.05], # dice, bce
    'accum_iter': 1,
    'weight_decay': 1e-6,
    'num_workers': 12,
    'device': 'cuda',
...
train_transforms = A.Compose([
    ...
    A.Cutout(num_holes=2, max_h_size=int(0.08*cfg['img_size']), max_w_size=int(0.08*cfg['img_size']), p=0.7),
...
def prepare_dataloader(train_ids, val_ids):
    ...
    val_dataset = HuBMAPDataset(val_ids, train=False, tfms=A.Resize(512, 512))
    train_loader = DataLoader(train_dataset, batch_size=cfg['train_batch_size'], shuffle=True, num_workers=cfg['num_workers'], drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size=cfg['val_batch_size'], shuffle=False, num_workers=cfg['num_workers'], drop_last=True)
...
def validate(fold, val_loader, net, criterion):
    ...
    with torch.no_grad():
        ...
        if val_dice / (batch_idx + 1) > best_dice:
            ...
            torch.save(state, f'pretrained/pretrained-{model_name}-{encoder_name}-{img_size}-fold{fold}-dice.pth')
...
def main_loop(resume, adam):
    ...
    ids = np.array(os.listdir(images_path))
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        net = smp.DeepLabV3Plus(encoder_name=cfg['encoder_name'], encoder_weights='imagenet', classes=1, activation=None, aux_params=aux_params)
        if resume:
            model_name = cfg['model_name']
            encoder_name = cfg['encoder_name']
            img_size = cfg['img_size']
            checkpoint = torch.load(f'pretrained/pretrained-{model_name}-{encoder_name}-{img_size}-fold{fold}-dice.pth')
        ...
        for epoch in range(cfg['epochs']):
            print('\nEpoch: %d' % epoch)
            if epoch >= 10:
                # print('using new weights and docal loss...\n')
                cfg['lr'] = 1e-4
                cfg['min_lr'] = 8e-6
                cfg['loss_weights'] = [0.9, 1.0]
                criterion = DiceFocalLoss()

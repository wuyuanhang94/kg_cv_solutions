...
train_transforms = A.Compose([
    ...
    A.ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.2, rotate_limit=15, p=0.2, border_mode=cv2.BORDER_REFLECT),
    A.Transpose(p=0.3),
    ...
    A.Cutout(num_holes=2, max_h_size=int(0.08*cfg['img_size']), max_w_size=int(0.08*cfg['img_size']), p=0.2),
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        net = smp.Unet(encoder_name=cfg['encoder_name'], encoder_weights='imagenet', classes=1, activation=None, aux_params=aux_params)
        if resume:
            model_name = cfg['model_name']
            encoder_name = cfg['encoder_name']
            img_size = cfg['img_size']

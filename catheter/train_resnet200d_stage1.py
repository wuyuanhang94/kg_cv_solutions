...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'teacher/catheter-{model_arch}-384teacher-fold{fold}-auc.pth'))
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            ...
            model_arch = cfg['model_arch']
            checkpoint = torch.load(f'teacher/catheter-{model_arch}-384teacher-fold{fold}-auc.pth')
            ...
            net.load_state_dict(pretrained_checkpoint['model'])
            net = torch.nn.DataParallel(net, device_ids=[0])
            net = net.cuda()

...
def get_train_transforms():
    return Compose([
            ...
            RandomResizedCrop(cfg['img_size'], cfg['img_size'], scale=(0.3, 1.0)),
            Transpose(p=0.5),
...
def save_model(net, fold, mode='auc', auc=.0, loss=.0):
    global best_auc
    global best_loss
    ...
    if mode == 'auc':
        ...
        torch.save(state, os.path.join(project_path, f'checkpoint/catheter-b4-fold{fold}-auc.pth'))
        best_auc = auc
    else:
        torch.save(state, os.path.join(project_path, f'checkpoint/catheter-b4-fold{fold}-loss.pth'))
        best_loss = loss
...
def validate(val_loader, net, criterion, fold, val_labels):
    ...
    if auc > best_auc:
        save_model(net, fold, 'auc', auc, test_loss)
    if test_loss < best_loss:
        save_model(net, fold, 'loss', auc, test_loss)
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            net = EfficientNet.from_name(cfg['model_arch'], num_classes=11, image_size=cfg['img_size']).to(device)
            net._fc = nn.Linear(net._fc.in_features, 11)
            net = torch.nn.DataParallel(net, device_ids=[0])
            net = net.cuda()            
            
            checkpoint = torch.load(f'checkpoint/catheter-b4-fold{fold}-auc.pth')
            ...
            net = EfficientNet.from_pretrained(cfg['model_arch'], num_classes=11, image_size=cfg['img_size']).to(device)
            net._fc = nn.Linear(net._fc.in_features, 11)
            net = torch.nn.DataParallel(net, device_ids=[0])
            net = net.cuda()

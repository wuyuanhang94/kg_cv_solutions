...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/catheter-b7-fold{fold}-auc.pth'))
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        if resume:
            ...
            net.classifier = nn.Linear(net.classifier.in_features, 11)
            ...
            net = timm.create_model(cfg['model_arch'], pretrained=True, num_classes=11).to(device)
            net.classifier = nn.Linear(net.classifier.in_features, 11)
            net = torch.nn.DataParallel(net, device_ids=[0])
            net = net.cuda()

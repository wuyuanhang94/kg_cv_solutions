...
class CustomLoss(nn.Module):
    ...
    def forward(self, teacher_features, features, y_pred, labels):
        ...
        cls_loss = FocalLoss()(y_pred, labels)
...
def main_loop(resume, adam):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        teacher = CustomResNet200D(pretrained=False)
        teacher = torch.nn.DataParallel(teacher, device_ids=range(cfg['device_num']))
        teacher = teacher.cuda()
        img_size = cfg['img_size']
        teacher_fold_path = os.path.join(teacher_path, f'catheter-resnet200d-{img_size}teacher-fold{fold}-auc.pth')
        ...
        if resume:
            net = CustomResNet200D(pretrained=False)
            ...
            pretrained_path = './resnet200d_320_chestx.pth'
